...
def main():
    ...
    test_generator = DataGenerator(dataframe=df_test, batch_size=128, target_size=target_size)

    # Evaluate models and prepare submission
    true_labels_test, pred_labels_test = evaluate_models(selected_models, test_generator)
    majority_votes = mode(pred_labels_test, axis=0).mode[0]
    ...
    final = pd.concat([df_test['image_path'].reset_index(drop=True), pd.Series(y_predicted)], axis=1)
